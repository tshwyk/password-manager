...
#define SUCCESS SGX_SUCCESS
#define ERROR SGX_ERROR_UNEXPECTED

/* Structures for managing datas */
...
sgx_status_t new_vault(char *passphrase) {
    ...
    if (passphrase == NULL) return ERROR;
    /* Vault already initialized */
    if (vault_pass != NULL) return ERROR;
...
sgx_status_t load_vault(uint8_t *vault_data, uint32_t vault_size,
    ...
    uint32_t buf_size = sgx_get_encrypt_txt_len(
        (sgx_sealed_data_t *)vault_data);
    ...
    uint32_t len = 0;
    ret = sgx_unseal_data((sgx_sealed_data_t *)vault_data, NULL, 0, buf,
        &buf_size);
...
sgx_status_t create_account(char *name, char *user, char *pass) {
  /* Name, user or pass cannot be null */
    if (name == NULL || user == NULL || pass == NULL) return ERROR;
    /* Vault not initialized */
    if (vault_pass == NULL) return ERROR;
...
sgx_status_t get_username(char *name, char *user, uint32_t size) {
    ...
    if (name == NULL || user == NULL || size < 2) return ERROR;
    /* Vault not initialized */
    if (vault_pass == NULL) return ERROR;

    /* Get the matching account */
    account_t *account = get_account(name);
    if (account == NULL || size < strlen(account->user) + 1) return ERROR;
...
sgx_status_t get_password(char *name, char *pass, uint32_t size) {
    ...
    if (name == NULL || pass == NULL || size < 2) return ERROR;
    /* Vault not initialized */
    if (vault_pass == NULL) return ERROR;

    /* Get the matching account */
    account_t *account = get_account(name);
    if (account == NULL || size < strlen(account->pass) + 1) return ERROR;
